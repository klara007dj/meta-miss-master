...
export default function HomeDashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.get("/candidates/top?type=MISS&limit=6").catch(() => null),
      api.get("/candidates/top?type=MASTER&limit=6").catch(() => null),
    ]).then(([missRes, masterRes]) => {
      setTopMiss(missRes?.data?.data || []);
      setTopMaster(masterRes?.data?.data || []);
    ...
    <div className="page-content fade-up">
      ...
      <div className="stat-grid">
        <div className="stat-box">
          ...
          <div style={{ fontSize: "1.5rem", fontWeight: 800, color: "var(--text)", lineHeight: 1 }}>{stats?.totalCandidates ?? "—"}</div>
      ...
      {/* ✅ 3 états : chargement / vide / liste */}
      {loadingCandidates ? (
        /* Skeleton — seulement pendant le vrai chargement */
        <div style={{ display: "flex", gap: 10, padding: "0 16px 8px", overflowX: "auto" }}>
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="shimmer" style={{ width: 120, height: 160, borderRadius: 16, flexShrink: 0 }} />
          ))}
        </div>
      ) : topAll.length === 0 ? (
        /* ✅ État vide — pas de skeleton infini */
        <div style={{
          margin: "0 16px 24px",
          padding: "32px 20px",
          borderRadius: 16,
          ...
          <div style={{ fontSize: "0.82rem", color: "var(--text-muted)", lineHeight: 1.5 }}>
            Le concours est ouvert ! Sois le premier à poser ta candidature.
          </div>
          <Link
              ...
              <Link key={c.id} href={`/candidates/${c.id}`} style={{ textDecoration: "none", flexShrink: 0, scrollSnapAlign: "start" }}>
                <div style={{ width: 120, borderRadius: 16, overflow: "hidden", position: "relative", height: 160, boxShadow: "0 2px 12px rgba(0,0,0,0.10)" }}>
                  ...
                  <div style={{
                    ...
                    background: i === 0 ? "#F59E0B" : i === 1 ? "#9CA3AF" : i === 2 ? "#B45309" : "#2563EB",

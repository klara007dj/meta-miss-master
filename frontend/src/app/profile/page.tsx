  ...
  return (
    <div className="page-content fade-up">
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "24px 20px 32px" }}>
        ...
        <div style={{ fontWeight: 800, fontSize: "1.05rem", color: "var(--text)" }}>{user?.name || "Sophie Diop"}</div>
        <div style={{ fontSize: "0.78rem", color: "var(--text-muted)", marginTop: 2 }}>{user?.email || "sophie.diop@email.com"}</div>
      </div>
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        {/* Bouton Sponsoriser */}
        <a
          href={`https://wa.me/237694600007?text=${encodeURIComponent("Bonjour, je souhaite soutenir et sponsoriser l'événement MetaMiss Master 2025. Merci de me contacter pour les détails.")}`}
          target="_blank"
        ...
        {/* Déconnexion */}
        <button onClick={() => setShowLogoutConfirm(true)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 0", borderBottom: "1px solid var(--border-light)", width: "100%", background: "none", border: "none", cursor: "pointer" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      {isAuthenticated && user?.role === "ADMIN" && (
        <div style={{ padding: "16px 16px 0" }}>
          <Link href="/admin" className="btn-blue" style={{ fontSize: "0.85rem" }}>{t.adminLink}</Link>
        </div>
      )}

      {!isAuthenticated && (
        <div style={{ padding: "20px 16px 0", display: "flex", flexDirection: "column", gap: 10 }}>
          <Link href="/login" className="btn-blue">{t.loginBtn}</Link>
          <Link href="/login?tab=register" className="btn-outline">{t.createAccount}</Link>
        </div>
      )}
